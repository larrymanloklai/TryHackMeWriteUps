...
- Tools: nmap, ffuf, cyberchef, ssh, ssh-keygen, steghide, stegseek, zip2john, john
...
- with the given IP a basic port discovery was done by nmap `nmap -sS ip`. after ports were discovered, additional switches were used to reveal details.
...
- FFUF was started to find sub directories. BURP was started record details while I was enumerating manually.
...
- Efford was make trying to check any client side filtering - negative. So it would be server side filtering. Here were the commands I trided.
  - worked: whoami, groups, id, /bin/ls
  - didnt work: ls, cat, /bin/cat index.php - not sure why it didn't work.
...
- Enumeration was done again. There was an interesting file `/var/www/files/index.php` revealed a root username password for mySQL dbname webportal. Also another file '/var/www/files/account.php` revealed the hash type stored would be md5.
...
- Next, hashcat was used to brute force cracking the password `$ hashcat -m 0 -a 0 hash.txt /usr/share/wordlists/rockyou.txt`, and `$ hashcat -m 0 hash.txt --show`
...
- The clue was look in the dark to find your answer. The hacker looke 'dark', and by looking at the folder /var/www/files again under images there are two files. The jpeg was checked with steghide and a zipped file was found within.
